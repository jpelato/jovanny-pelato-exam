		...
		<v-col cols="12" class="mt-10">
			...
			<v-text-field v-model="keyword" variant="solo-filled" flat label="Enter a keyword to view the most recent video." hide-details="auto" clearable @click:clear="showSearch = false" @keyup.enter="searchByKeyTag">
				...
				<template v-slot:append>
					<v-btn class="ma-0 pa-0 ml-n5" height="56" width="64" color="teal" rounded="0" dark variant="elevated" @click="searchByKeyTag"> <v-icon size="x-large" icon="mdi-movie-search-outline"/></v-btn>
				</template>
			</v-text-field>
			</v-card>
		</v-col>
		...
		<v-col cols="12" v-if="!showSearch && channel_added_to_library.length > 0 && !channel_on_view.channelId">
			<v-row>
				<v-col class="d-flex child-flex" v-for="(videoObject, i) in channel_added_to_library" :key="i" cols="12" sm="6" xs="12" lg="6" md="6">
					<libraryCard @getChannelVideo="getChannelVideo" :videoObject="videoObject" />
				</v-col>
			</v-row>
		</v-col>
		<v-col cols="12" v-if="!showSearch && search_channel_video_list.length > 0">
			<v-row>
...
<script>
	...
	export default {
		...
		data: () => ({
			...
			playlist_id:'UUWJ2lWNubArHWmf3FIHbfcQ',
			channel_video_list:[],
		...
		methods:{
			...
			addToLibrary(){
				this.axios.post('/youtube_channels.php',this.channel_on_view)
				.then(function (response) {
			...
			checkLibrary(){
				let scope = this;
				this.axios.get('/youtube_channels.php')
				.then(function (response) {
					let items = response.data
					for (let x in items) {
						var obj = {
							channelId:items[x].kind,
							title:items[x].title,
			...
			libChannelVideo(){
				let scope = this;
				this.axios.get('/youtube_channel_videos.php')
				.then(function (response) {
					let items = response.data
					console.log(items);
				})
			...
			sycVideo(){
				this.axios.post('/sync_youtube_channel.php',this.channel_video_list)
				.then(function (response) {
					console.log(response)
				})
				.catch(function (error) {
